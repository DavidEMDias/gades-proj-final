...
def build_query(bq_dataset, bq_table, sql_path, gcp_conn_id="google_cloud_default"):
    ...
    try:
        ...
        print("Último updated_at na tabela BQ:", row.last_updated)
    except NotFound:
...
def decide_next_task(bucket_name, object_name, gcp_conn_id, **kwargs):
    ...
    try:
        hook.list(bucket_name=bucket_name, max_results=1)  # tenta aceder o bucket
    except NotFound:
        print(f"Bucket '{bucket_name}' não existe ou não é acessível.")
        return "skip_load"
    
    # Verifica se o objeto existe dentro do bucket
    if hook.exists(bucket_name=bucket_name, object_name=object_name):
        print(f"Objeto '{object_name}' encontrado no bucket '{bucket_name}'.")
        return "load_data"
    else:
